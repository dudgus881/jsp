...
public class LoginController {
	...
	public String json() {
		...
		return "jsonView"; // <bean id="jsonView" class="MappingJackson2JsonView">
				...
				//	  prefix, surfix, 설정에 때라 /WEB-INF/views/jsonView.jsp
...
//	@GetMapping()
	public String getView() {
		logger.debug("LoginContoller.getView()");
		return "login/view";
	}
	
	//파라미터 이름과 동일한 이름의 메서드 인자를 선언하면
	//스프링 프레임워크가 자동으로 바인딩 해준다.
	//값을 담을수 있는 객체를 메서드 인자로 선언한 경우에도 필드명과 파라미터 명이
	// 동일하면 자동으로 바인딩 처리를 해준다.
	// 이때 값을 담는 객체를 스프링 프레임워크에서는 command 객체라고 명명한다.
	
	//Model : view객체에서 응답을 생성할때 참조할 데이터를 담는 객체
	//		  jsp/Servlet 기반의 request 역활을 담당
	...
	public String process(String userid, String pass, MemberVo memberVo,
		...
		//db에서 조회한 사용자 정보가 존재하지않으면 ==> login.jsp로 이동

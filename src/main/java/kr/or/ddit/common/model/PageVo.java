...
public class PageVo {
	...
	//인자있는 생성자를 선언하게 되면 default 생성자가(인자가 없는 생성자) 생성이 안됨
	//대다수의 framwork에서는 인자가 없는 생성자를 요구하는 경우가 많음)
	//famework에서 해당 클래스의 인스턴스를 생성해야하는데 인자가 있으면 어떻게 만들어야하는지 알수가 없는 경우가 대부분
	//spring, mybatis 를 사용할 경우 기본 생성자가 생성되게끔 코드를 작성

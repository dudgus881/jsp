...
public class RangerServiceTest {
	...
	//ex)	회원 등록 dao메서드를 생성시
	//		정상적인 값을 입력해서 db 테이블에 데이터가 정상적으로 입력되는 성공 케이스
	//		비정상적인 값(회원 id중복)을 입력하여 db 테이블에 데이터가 입력되지 않는 실패 케이스
	
	//테스트 메서드는 테스트 하려고하는 메서드 이름 + 접이어Test
	...
	// 이클립스에서 @Test 메서드가 붙은 메서드를 싱행해주기 때문(junit에서 실행)
	
	// maven 배포시에도 @Test 메서드가 붙은 메서드가 통과가 되어야 빌드 사이클이 진행이 된다.
